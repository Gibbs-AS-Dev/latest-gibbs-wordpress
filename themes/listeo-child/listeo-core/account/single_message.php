...
if( isset( $_GET["action"]) && $_GET["action"] == 'view' )  {
	...
	$current_user_id = $cr_user;
	
	// get this conversation data
	$this_conv = $messages->get_conversation($conversation_id);	
	//echo "<pre>"; print_r($this_conv); die;
	if(!$this_conv) { ?>
		<h4><?php esc_html_e('This message does not exists.','listeo_core'); ?></h4>
		<?php return;
	}
	if($current_user_id == (int)$this_conv[0]->user_1 || $current_user_id == (int)$this_conv[0]->user_2 || get_current_user_id() == (int)$this_conv[0]->user_1 || get_current_user_id() == (int)$this_conv[0]->user_2) :

		// mark this message as read
		$messages->mark_as_read($conversation_id);	
		
		// set who is adversary on that converstation
		$adversary = ($this_conv[0]->user_1 == $current_user_id || $this_conv[0]->user_1 == get_current_user_id()) ? $this_conv[0]->user_2 : $this_conv[0]->user_1 ;
		...
		if($this_conv[0]->referral && $this_conv[0]->referral != ""){
			$booking_string = explode("_", $this_conv[0]->referral);

			if(isset($booking_string[1])){
				$booking_id = $booking_string[1];
			}
		}
		...
		<div class="messages-container margin-top-0">
			<div class="messages-headline">
				<h4><?php echo esc_html($name); ?> <?php if($booking_id != "") : ?><span>Booking ID: <?php echo esc_html($booking_id);  ?></span><?php endif; ?><?php if($referral) : ?><span>Listing: <?php echo esc_html($referral);  ?></span><?php endif; ?></h4>
									...
									<a href="<?php echo esc_url($message_url) ?>">
										...
										<div class="message-by">
											...
											<p>
												<?php 
													if(!empty($last_msg)){
													echo ( $last_msg[0]->sender_id == get_current_user_id() ) ? '<i class="fas fa-share" ></i>' : '<i class="fas fa-reply"></i>';
													?> <?php echo esc_html($last_msg[0]->message); ?>
												<?php } ?>
											</p>
										</div>
									</a>
				...
				<div class="message-content">
					<div class="message-bubbles">
						<?php
							...
							foreach ($conversation as $key => $message) { 
								...
								?>
								<div class="message-bubble sdsdd <?php if( get_current_user_id() == (int) $message->sender_id || $group_admin == (int) $message->sender_id) echo esc_attr('me'); ?>">
									<div class="message-avatar"><a href="<?php echo esc_url(get_author_posts_url($message->sender_id)); ?>">
										<?php echo get_avatar($message->sender_id, '70') ?></a>
										<p><?php echo $user_data->display_name;?></p>
									</div>
									<div class="message-text" style="display: block;"><?php echo wpautop(esc_html($message->message)); ?></div>
								</div>
							<?php }
						?>
					...
					<div class="message-reply">
						<form action="" id="send-message-from-chat">
							...
							<button class="button"><?php esc_html_e('Send melding', 'listeo_core'); ?></button>
						</form>
					</div>
