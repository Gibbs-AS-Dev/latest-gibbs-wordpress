<?php
	...
	if(empty($ids[0]) && $ids[0] == 0){ ?>
		<a href="<?php the_permalink(); ?>" target="_blank" style="z-index:2;position:absolute;height:100%;"><img class="fade-img" style="z-index:1;" src="<?php echo esc_attr(get_listeo_core_placeholder_image()); ?>"></a>
	<?php } else {
		...
		<div style="height:100%;" class="search_slide" id="id-<?php echo $id; ?>">
			...
			if(!empty($gallery)) :
				?> <a href="<?php the_permalink(); ?>" target="_blank" style="z-index:2;position:absolute;height:100%;width:100%;"><?php
				foreach ( (array) $gallery as $attachment_id => $attachment_url ) {
					$image = wp_get_attachment_image_src( $attachment_id,'high', false, false); ?>
