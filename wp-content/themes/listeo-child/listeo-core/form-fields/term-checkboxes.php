...
if($dynamic_features == 'dynamic'){ ?>
	<div class="<?php echo esc_attr($dynamic_features); ?> checkboxes in-row listeo_core-term-checklist listeo_core-term-checklist-<?php echo $key ?> <?php echo $required;?>">
...
<?php } else { ?>
	...
	<?php
		...
		foreach ($terms as $key => $category) {
			echo '<input '.$required.' value="' . $category->term_id . '" type="checkbox" name="tax_input['.$taxonomy.'][]" id="in-'.$taxonomy.'-' . $category->term_id . '"' .
