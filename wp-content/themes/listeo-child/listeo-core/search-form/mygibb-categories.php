		...
		<div class="panel-dropdown-content checkboxes">
			...
			<?php
				...
				<?php for ($x = 0; $x < count($arrOfDepths)-1; $x++) { ?>
					...
					$ancestor = get_ancestors(get_term_by('slug', $value->slug, $data->taxonomy)->term_id, $data->taxonomy); ?>
					...
					<?php for($i = 0; $i < count($arrOfDepths[$x]); $i++) {
						...
						$hasTermChildren = count(get_term_children($value->term_id, $data->taxonomy)) > 0;
