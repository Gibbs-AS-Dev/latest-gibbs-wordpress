...
function SlotBookingConfirmation({ userLoggedIn, bookingToken, apiUrl, cr_user_id, setCrUserId, pluginUrl, homeUrl, prevBookingData, handleBackToBooking, handleDinteroPayment, handleNetsEasyPayment, handleWaitingPayment}) {
  ...
  const handleSocialLogin = (provider) => {
    ...
    // Or trigger OAuth popup
    const width = 500;
    const height = 600;
    const left = (window.screen.width / 2) - (width / 2);
    const top = (window.screen.height / 2) - (height / 2);
    const popup = window.open(redirectUrl, 'socialLogin', `width=${width},height=${height},left=${left},top=${top}`);

    // Handle the popup response using a polling mechanism
    const checkPopupResponse = setInterval(() => {
      try {
        
        if (popup && popup.closed) {
          clearInterval(checkPopupResponse);
          // Handle the response if the popup is closed
        ...
        if(popup){
          ...
          const responseData = url.searchParams.get('code');  // Or whatever data is returned by PHP

          // If we received the expected response, close the popup and handle it
          if (responseData) {
            clearInterval(checkPopupResponse);
            popup.close();
            
            // Exchange the authorization code for an access token
            exchangeCodeForToken(responseData, provider);
          }

          const errorData = url.searchParams.get('error');
          if (errorData) {
            clearInterval(checkPopupResponse);
            popup.close();
        ...
        // Check the URL of the popup to see if the response is available
       // if (popup && popup.location.href.includes('endpoint.php')) {
          
       // }
      } catch (error) {
        // Handle any errors (e.g., cross-origin issues)
      }
    }, 1000);
  };
            ...
            <div className={styles.contactSection}>
              ...
              <div className={styles.socialLoginButtons} style={{ display: 'none' }}>
                ...
                >
                  ...
                  <span className={styles.socialBtnText}>{Ltext("Vipps")}</span>
                </button>
