...
function GibbsCustomer({ apiUrl, user_token, owner_id }) {
  ...
  const [isEditCustomerNotesOpen, setIsEditCustomerNotesOpen] = useState(false);
  const [editingCustomerNotes, setEditingCustomerNotes] = useState(null);
  ...
  const allColumns = [
    ...
    {
      key: 'customer_notes',
      header: Ltext('CUSTOMER NOTES'),
      sortable: true,
      thStyle: { width: '12%' },
      render: (row) => {
        if (row.isUsergroup) return '—';
        const notes = row.customer_notes || row.abdis_notes || '';
        ...
        return (
          <div
            style={{
              ...
              padding: '10px 14px',
              backgroundColor: hasNotes ? '#f9fafb' : '#fafafa',
              border: hasNotes ? '1px solid #e5e7eb' : '1px dashed #d1d5db',
              borderRadius: '8px',
              fontSize: '13px',
              lineHeight: '1.6',
            ...
            onClick={() => {
              setEditingCustomerNotes(row);
              setIsEditCustomerNotesOpen(true);
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.backgroundColor = hasNotes ? '#f3f4f6' : '#f0f0f0';
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.backgroundColor = hasNotes ? '#f9fafb' : '#fafafa';
              e.currentTarget.style.borderColor = hasNotes ? '#e5e7eb' : '#d1d5db';
              e.currentTarget.style.borderStyle = hasNotes ? 'solid' : 'dashed';
              e.currentTarget.style.color = hasNotes ? '#374151' : '#9ca3af';
              e.currentTarget.style.boxShadow = 'none';
              e.currentTarget.style.transform = 'translateY(0)';
            }}
            title={hasNotes ? Ltext('Click to edit notes') : Ltext('Click to add notes')}
          >
            {hasNotes ? (
              notes
            ) : (
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <EditCustomerNotes
        isOpen={isEditCustomerNotesOpen}
        onClose={() => {
          setIsEditCustomerNotesOpen(false);
          setEditingCustomerNotes(null);
        }}
        apiUrl={apiUrl}
        user_token={user_token}
        owner_id={owner_id}
        customer={editingCustomerNotes}
        onSuccess={() => {
          // Refresh current page after successful edit
          fetchCustomers(page);
        }}
