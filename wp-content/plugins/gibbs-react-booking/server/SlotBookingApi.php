...
class SlotBookingApi extends CoreApiHandler {
    ...
    public function submitSlotBooking($data) {
        ...
        if($payment_method != "dintero" && $payment_method != "dibs_easy" && $payment_method != "cod"){
            ...
            if(isset($status_data['order_id']) && $status_data['order_id'] > 0){
                if(isset($status_data['order_received_url']) && $status_data['order_received_url'] != ""){
                    ...
                    if($payment_method == "dintero"){
                        $payment_data = $this->dinteroPayment($status_data['order_id'],"dintero");
                    }else if($payment_method == "dibs_easy"){
                         $payment_data = $this->dibsEasyPayment($status_data['order_id'],"dibs_easy");
                    }else if($payment_method == "cod"){

                        $payment_data = $this->codPayment($status_data['order_id'],"cod");
    ...
    public function dinteroPayment($order_id, $payment_method_id){
        ...
        if (!$order) {
            CoreResponse::error('Order not found', 400);
        }
    ...
    public function dibsEasyPayment($order_id, $payment_method_id){
    ...
    public function codPayment($order_id, $payment_method_id){
    ...
    public function getSlotPrice($data, $return_access = false) {
        ...
        // echo "<pre>";
        // print_r($remaining_slots);
        // echo "</pre>";
        // die();

        if($remaining_slots == 0 || $remaining_slots < 0){
            CoreResponse::error('Slot is not available', 400);
        }
        if($remaining_slots < $adults){
