...
// has_listing_gallery
	...
	public function dler() {
		...
		try {
			...
			if(isset($this->listing_id) && $this->listing_id != "" && $this->listing_id > 0){
				$cr_post = get_post($this->listing_id);

				if(isset($cr_post->ID)){
					if((int) $cr_post->post_author != (int) $group_admin){
						throw new Exception( __( 'You dont have access.', 'listeo_core' ) );
						return;
					}
				}
			...
			if(isset($_POST["slots_type"]) && $_POST["slots_type"] == "standard"){
				if(isset($_POST["standard_from_day"]) && !empty($_POST["standard_from_day"])){
					...
					for ($ik=0; $ik < count($_POST["standard_from_day"]) ; $ik++) { 
						...
						if($_POST["standard_to_time"][$ik] == "00:00"){
							$_POST["standard_to_time"][$ik] = "23:59";
						}
						...
						$standard_slot_label = $_POST["standard_slot_label"][$ik];
						$standard_start_date = $_POST["standard_start_date"][$ik];
						$standard_end_date = $_POST["standard_end_date"][$ik];
						...
						$standard_patern[$ik]["standard_slot_label"] = $_POST["standard_slot_label"][$ik];
						$standard_patern[$ik]["standard_start_date"] = $_POST["standard_start_date"][$ik];
						$standard_patern[$ik]["standard_end_date"] = $_POST["standard_end_date"][$ik];
						...
						if(!empty($intervals)){
							foreach($intervals as $interval){
								...
								$to_tm = $interval["end"];
								$slotss[] =  $standard_from_day."|".$fr_tm."|".$standard_from_day."|".$to_tm."|".$standard_slot_price."|".$standard_slots."|".$kinc."|".$standard_closed."|".$standard_all_slot_price."|".$standard_slot_label."|".$standard_start_date."|".$standard_end_date;
								$kinc++;
				...
				if(isset($_POST["from_day"]) && !empty($_POST["from_day"])){
					...
					for ($i=0; $i < count($_POST["from_day"]) ; $i++) { 
						 ...
						 $slotss[] =  $_POST["from_day"][$i]."|".$_POST["from_time"][$i]."|".$_POST["to_day"][$i]."|".$_POST["to_time"][$i]."|".$_POST["slot_price"][$i]."|".$_POST["slots"][$i]."|".$i."|0|".$_POST["all_slot_price"][$i]."|".$_POST["slot_label"][$i]."|".$_POST["start_date"][$i]."|".$_POST["end_date"][$i];
...
//print_r($intervals);
			...
			if(isset($_POST["enable_slot_duration"]) && $_POST["enable_slot_duration"] == "on"){
				update_post_meta($this->listing_id,"enable_slot_duration",$_POST["enable_slot_duration"]);
			}else{
				update_post_meta($this->listing_id,"enable_slot_duration","");
			}
			if(isset($_POST["hide_slotv2_widget_border"]) && $_POST["hide_slotv2_widget_border"] == "on"){
				update_post_meta($this->listing_id,"hide_slotv2_widget_border",$_POST["hide_slotv2_widget_border"]);
			}else{
				update_post_meta($this->listing_id,"hide_slotv2_widget_border","");
			}
			...
			if(isset($_POST["first_booking_minimum_guests"])){
				update_post_meta($this->listing_id,"first_booking_minimum_guests",$_POST["first_booking_minimum_guests"]);
			}else{
				delete_post_meta($this->listing_id,"first_booking_minimum_guests");
			}
			if(isset($_POST["_internal_booking_email_list"])){
				update_post_meta($this->listing_id,"_internal_booking_email_list",$_POST["_internal_booking_email_list"]);
			}else{
				delete_post_meta($this->listing_id,"_internal_booking_email_list");
			}
			if(isset($_POST["hide_booking_message"])){
				update_post_meta($this->listing_id,"hide_booking_message",$_POST["hide_booking_message"]);
			}else{
				delete_post_meta($this->listing_id,"hide_booking_message");
			}

			if(isset($_POST["default_customer_type"])){
				update_post_meta($this->listing_id,"default_customer_type",$_POST["default_customer_type"]);
			}else{
				delete_post_meta($this->listing_id,"default_customer_type");
			}

			if(isset($_POST["default_payment_method"])){
				update_post_meta($this->listing_id,"default_payment_method",$_POST["default_payment_method"]);
			}else{
				delete_post_meta($this->listing_id,"default_payment_method");
			}


			if(is_array($_POST) && key_exists("season_status",$_POST)){
				update_post_meta($this->listing_id,"season_status",$_POST["season_status"]);
			}else{
				delete_post_meta($this->listing_id,"season_status");
			}

			if(is_array($_POST) && key_exists("season_name",$_POST) && !empty($_POST["season_name"])){

				$season_discount_data = array();
				

				foreach($_POST["season_name"] as $key => $season_name){

					if(trim($season_name) != ""){
						
						if($_POST["season_price_percent"][$key] == ""){
							throw new Exception( "Season price percent is required" );
							exit;
						}
						if($_POST["season_price_from"][$key] == ""){
							throw new Exception( "Season price from date is required" );
							exit;
						}
						if($_POST["season_price_to"][$key] == ""){
							throw new Exception( "Season price to date is required" );
							exit;
						}
						
						$season_discount_data[] = array(
							"season_name" => $season_name,
							"season_price_percent" => $_POST["season_price_percent"][$key],
							"season_price_from" => $_POST["season_price_from"][$key],
							"season_price_to" => $_POST["season_price_to"][$key],
							"season_price_active" => $_POST["season_price_active"][$key],
						);
					}	
				}

				update_post_meta($this->listing_id,"season_discount_data",$season_discount_data);
			}else{
