		...
		if ( is_post_type_archive( 'listing' ) || is_author() || is_tax('listing_category') || is_tax('listing_feature') || is_tax('event_category') || is_tax('service_category') || is_tax('rental_category') || is_tax('region')) {
		 ...
		 //    }
            ...
            if(isset($_REQUEST["user"])){
				$query->set('author__in', $_REQUEST["user"]);	
			}
...
//check to apply only for events
			...
			if( $active_group_id != ""){
				...
				if(get_current_user_ID() && get_current_user_ID() != ""){
		            ...
		            //echo "<pre>"; print_r( $user_group_data_rows ); die;
...
public function ajax_get_listings() {
		...
		if(isset($_REQUEST["user"])){
			$query_args["author__in"] = $_REQUEST["user"];
		}
...
//				'class' 		=> $custom_class, 
		...
		$result['map_html'] = $this->get_all_listings($query_args,$style,$grid_columns,$template_loader);
	...
	public function get_all_listings($query_args,$style,$grid_columns,$template_loader){
...
//				'class' 		=> $custom_class, 
			...
			<div class="loader-ajax-container" style=""> <div class="loader-ajax"></div> </div>
				...
				while ( $listings->have_posts() ) {
					$listings->the_post();
					  ...
					  $template_loader->set_template_data( $style_data )->get_template_part( 'content-listing-map',"list" ); 	
					}
				?>
				<div class="clearfix"></div>
			</div>
  ...
  // 2 => string 'radius'
	...
	public function output_search_form( $atts = array() ){
		...
		<form action="<?php echo $action; ?>" id="listeo_core-search-form" class="<?php if($dynamic_filters == 'on') { echo esc_attr('dynamic'); }  ?> <?php if($dynamic_taxonomies == 'on') { echo esc_attr('dynamic-taxonomies'); }  ?>  <?php echo esc_attr($custom_class) ?> <?php echo esc_attr($ajax) ?>" method="GET">
		<?php } 
			...
			foreach ($search_fields as $key => $value) {
				...
				if(isset($value['place']) && $value['place'] == 'main') {
					...
					if($source == 'half') {
						...
						if($source == 'sidebar') { echo '<div class="row with-forms" id="listeo-search-form_'.$value['name'].'">'; }
							$template_loader->set_template_data( $value )->get_template_part( 'search-form/'.$value['type']);
