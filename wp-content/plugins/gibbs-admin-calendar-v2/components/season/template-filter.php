...
$filter_template_sql = "SELECT * from $filter_template_table where template_type = 'calender-season' AND user_id=".$admin_user_id;
$filter_template_data = $wpdb->get_results($filter_template_sql);

if(empty($filter_template_data)){
    Gibbs_Season_Calendar_Setup::saveCalenderFilter($admin_user_id);
    $filter_template_data = $wpdb->get_results($filter_template_sql);

    
}

$filter_template_type = "calender-season";
