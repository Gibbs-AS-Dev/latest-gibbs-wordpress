...
class Gibbs_Season_Calendar_API
{
    ...
    public function season_export_csv($bookings){
        ...
        $dir = GIBBS_CALENDAR_PATH . 'assets/csv/';
        $filename = 'export_booking-'.date("y-m-d")."-".time().'.csv';
        $csv_path = $dir . $filename;
        $csv_url = GIBBS_CALENDAR_URL . 'assets/csv/' . $filename;

        
        $existing_csv_files = glob($dir . '*.csv');
        foreach ($existing_csv_files as $old_file) {
            if (is_file($old_file)) {
                unlink($old_file);
            }
        }
    
        // Opprett tom fil først (dersom den ikke finnes)
        if (!file_exists($csv_path)) {
            $created = touch($csv_path); // oppretter fil
            
            chmod($csv_path, 0644); // sett rettigheter
        }
    
        // Åpne fil for skriving (overskriv innhold)
        $f = fopen($csv_path, 'w');
        if (!$f) {
            wp_send_json_error(['message' => 'Kunne ikke åpne fil for skriving.']);
            return;
        }
    
        // Skriv CSV-headere
        fputcsv($f, ['Start date', 'End date', 'Comment', 'Client', 'Listing', 'Team', 'Status', 'Recurrence rule']);
    
        global $wpdb;
        $team_table = $wpdb->prefix . 'team';
        $listing_table = $wpdb->prefix . 'posts';
    
        foreach ($bookings as $json_value) {
            $team_id = intval($json_value["team"]["value"]);
            $team_name = $wpdb->get_var($wpdb->prepare("SELECT name FROM `$team_table` WHERE id = %d", $team_id));
    
            $listing_id = intval($json_value["gymSectionId"]);
            $listing_title = $wpdb->get_var($wpdb->prepare("SELECT post_title FROM `$listing_table` WHERE ID = %d", $listing_id));
    
            fputcsv($f, [
                $json_value["start"],
                $json_value["end"],
                $json_value["description"],
                $json_value["client"]["text"],
                $listing_title ?: '',
                $team_name ?: '',
                $json_value["org_status"],
                $json_value["recurrenceRule"]
            ]);
        }
