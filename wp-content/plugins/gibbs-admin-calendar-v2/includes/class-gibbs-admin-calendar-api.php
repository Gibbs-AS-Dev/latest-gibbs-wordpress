...
class Gibbs_Admin_Calendar_API
{
    ...
    public static function action_init()
    {
        ...
        add_action('wp_ajax_getSlotData', array('Gibbs_Admin_Calendar_API', 'getSlotData'));
    }
    public function getSlotData(){
        ...
        $slotdata = get_post_meta($listing_id, "_booking_slots", true);

        $hasPrivateSlot = false;
        $slot_guest = 0;

        if(is_array($slotdata) && !empty($slotdata)){
            foreach ($slotdata as $key => $_slot) {
                $slott = explode("|", $_slot);
        ...
        if($hasPrivateSlot){
            $private_slot = $wpdb->get_row("SELECT * from bookings_calendar_meta where meta_key = 'private_slot'  AND booking_id = " . $booking_id);
            if(isset($private_slot->meta_value) && $private_slot->meta_value != ""){
                $private_slot = $private_slot->meta_value;
            }else{
                $private_slot = "";
            }
        }else{
        ...
        $data = array();
        $data["private_slot"] = $private_slot;
        $data["hasPrivateSlot"] = $hasPrivateSlot;
        $data["slot_guest"] = $slot_guest;
        return wp_send_json(array( 'error' => 0,'data' => $data));
    }
    ...
    public function check_refund(){
        ...
        if(isset($_POST['order_id'])){
                ...
                if(!empty($refunds)){
                        ...
                        if($charge_id_exist != "" && ($order_status == "completed" || $order_status == "refunded" || $order_status == "on-hold")){
    ...
    public function getEventAjaxData(){
        ...
        if(isset($_POST['booking_id'])){
            ...
            if(isset($record->booking_extra_data) && $record->booking_extra_data != "" && $record->booking_extra_data != null){
                $booking_extra_data = json_decode($record->booking_extra_data);
                if(isset($booking_extra_data->coupon_data) && $booking_extra_data->coupon_data != "" && $booking_extra_data->coupon_data != null){
                    $coupon_data[] = $booking_extra_data->coupon_data;
                }

                // echo "<pre>"; print_r($booking_extra_data); die;

                // if(isset($booking_extra_data->gift_data) && is_array($booking_extra_data->gift_data) && !empty($booking_extra_data->gift_data)){
                //     $gift_data = $booking_extra_data->gift_data;
                //     foreach($gift_data as $gift){
                //         $coupon_data[] = $gift->code;
                //     }
                // }
            }

            $data["coupon_data"] = implode(",",$coupon_data);
      ...
      // echo $booking_where; die;
        ...
        foreach ($booking_results as $record) {
            ...
            if(isset($record->booking_extra_data) && $record->booking_extra_data != "" && $record->booking_extra_data != null){
                $booking_extra_data = json_decode($record->booking_extra_data);
                if(isset($booking_extra_data->coupon_data) && $booking_extra_data->coupon_data != "" && $booking_extra_data->coupon_data != null){
                    $coupon_data_record[] = $booking_extra_data->coupon_data;
                }
            }
            ...
            $record_data = array(
                ...
                'coupon_data_record'        => $coupon_data_record,
            );
            ...
            foreach ($_POST['listings'] as $key => $listing) {
                ...
                $result = $wpdb->get_results("
                    ...
                    WHERE 
                        ...
                        AND bookings_author = {$wpm_client} 
                        AND listing_id = {$listing} 
                ...
                if(isset($_POST["private_slot"]) && $_POST["private_slot"] == "true"){
                    $wpdb->insert("bookings_calendar_meta",
                            array(
                                "meta_key" => "private_slot",
                                "meta_value" => $_POST["private_slot"],
                                "booking_id" => $bk_id
                            )
                        );
                }else{
    ...
    public static function wpm_update_record()
    {
       ...
       // echo "<pre>"; print_r($bookings_calendar_meta_data); die;
        ...
        $bookings_calendar_meta_sql = "SELECT * from bookings_calendar_meta where meta_key = 'private_slot'  AND booking_id = " . $_POST["id"];
        $bookings_calendar_meta_data = $wpdb->get_row($bookings_calendar_meta_sql);
        ...
        if(isset($bookings_calendar_meta_data->meta_value) && $bookings_calendar_meta_data->meta_value != ""){
            $wpdb->update("bookings_calendar_meta",
                array(
                    "meta_value" => $private_slot
                ),
                array("id" => $bookings_calendar_meta_data->id)
            );
        }else{
            $wpdb->insert("bookings_calendar_meta",
                array(
