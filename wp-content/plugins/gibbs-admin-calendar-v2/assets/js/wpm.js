...
(function ($) {
    ...
    async function fetchSlotData(resourceId,id){
        let datas = {
            "action": "getSlotData",
    ...
    async function fillPopup(event) {
       ...
       // jQuery(".payment_plus").show();
        ...
        jQuery(".slot_private_div").append('<div class="popup-loader"><div class="loaderdivv"></div></div>')
        jQuery(".slot_private_div").show();
        jQuery('.slot_private_switch_label').hide()
        


        const slotData = await fetchSlotData(eventResource,eventId);

        jQuery(".slot_private_div").find(".popup-loader").remove();
        jQuery('.slot_private_switch_label').show()

        if(slotData?.data?.hasPrivateSlot){
            jQuery(".slot_private_div").show();

            if(event?.amount_guest){
            }else{
                slot_guest = slotData?.data?.slot_guest;
            }

            if(slotData?.data?.private_slot && slotData?.data?.private_slot == "true"){
                $('#slot_private').mobiscroll('getInst').checked = true
            }else{
                $('#slot_private').mobiscroll('getInst').checked = false;
            }
    ...
    async function getEventAjaxData(event) {
        ...
        jQuery.ajax({
            ...
            success: function(resultData) {
                ...
                if (typeof resultData.data["coupon_data"] !== "undefined" && resultData.data["coupon_data"] != "") {
                    left_info_data.append('<p class="tooltip-custom customer_div">' +
                        '<b>Kupong:</b> <span>' + resultData.data["coupon_data"] + '</span>' +
                        '</p>');
                }
            },
            error: function(error) {
        ...
        renderScheduleEvent: function (data) {
            ...
            if (data.allDay) {
                ...
                if(event.custom_fields && event.custom_fields != "" && event.custom_fields == true && Array.isArray(calendarShowAdminIcons) && calendarShowAdminIcons.includes("custom_field")){
                    try {
                        icons += '<span  data-content="Bookinginformasjon" class="tooltip_info"><svg width="1em" height="1em xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512"><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M413.5 237.5c-28.2 4.8-58.2-3.6-80-25.4l-38.1-38.1C280.4 159 272 138.8 272 117.6V105.5L192.3 62c-5.3-2.9-8.6-8.6-8.3-14.7s3.9-11.5 9.5-14l47.2-21C259.1 4.2 279 0 299.2 0h18.1c36.7 0 72 14 98.7 39.1l44.6 42c24.2 22.8 33.2 55.7 26.6 86L503 183l8-8c9.4-9.4 24.6-9.4 33.9 0l24 24c9.4 9.4 9.4 24.6 0 33.9l-88 88c-9.4 9.4-24.6 9.4-33.9 0l-24-24c-9.4-9.4-9.4-24.6 0-33.9l8-8-17.5-17.5zM27.4 377.1L260.9 182.6c3.5 4.9 7.5 9.6 11.8 14l38.1 38.1c6 6 12.4 11.2 19.2 15.7L134.9 484.6c-14.5 17.4-36 27.4-58.6 27.4C34.1 512 0 477.8 0 435.7c0-22.6 10.1-44.1 27.4-58.6z"/></svg></span>';
                    } catch (e) {
                       // return false;
                    }
                ...
                if(event.coupon_data_record != undefined && Array.isArray(event.coupon_data_record) && event.coupon_data_record.length > 0){
                    
                    try {
                        icons += '<span data-content="Coupon" class="tooltip_info"><svg height="1em" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><!--!Font Awesome Free 6.7.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M64 64C28.7 64 0 92.7 0 128v64c0 8.8 7.4 15.7 15.7 18.6C34.5 217.1 48 235 48 256s-13.5 38.9-32.3 45.4C7.4 304.3 0 311.2 0 320v64c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V320c0-8.8-7.4-15.7-15.7-18.6C541.5 294.9 528 277 528 256s13.5-38.9 32.3-45.4c8.3-2.9 15.7-9.8 15.7-18.6V128c0-35.3-28.7-64-64-64H64zm64 112l0 160c0 8.8 7.2 16 16 16H432c8.8 0 16-7.2 16-16V176c0-8.8-7.2-16-16-16H144c-8.8 0-16 7.2-16 16zM96 160c0-17.7 14.3-32 32-32H448c17.7 0 32 14.3 32 32v192c0 17.7-14.3 32-32 32H128c-17.7 0-32-14.3-32-32V160z"/></svg></span>';
...
/*                 '<option value="timeline_year" class="fc-timelineYear-button fc-button fc-state-default" ' + (calendar_view_val === 'timeline_year' ? 'selected' : '') + '>Tidslinje år</option>' + */
    ...
    slotPrivateSwitchSelect = $('#slot_private').mobiscroll('getInst');
    slotPrivateSwitchSelect.change(function(){
        
        if(!editEvent){
            if(this.checked == true){

                $('.popup-event-guest').val(slot_guest);
                $('.popup-event-guest').change();
                
            }else{
                $('.popup-event-guest').val(1);
                $('.popup-event-guest').change();
            }
        }    
   ...
   // console.log({ filter_locations })
    ...
    async function add_booking(data) {
        try {
            // Prepare data
            ...
            if(jQuery("#slot_private").val() == "on"){
                data.private_slot = "true";
            }else{
                data.private_slot = "false";
            }
            ...
            const response = await new Promise((resolve, reject) => {
                $.post(WPMCalendarV2Obj.ajaxurl, data, resolve).fail(reject);
            });
            ...
            showToastMessage("Noe gikk galt under bestillingen.", "error");
        } finally {
            hideLoader();
        }
    }
