...
if($active_group_id != ""){
	...
	$sql_application_data = "select *  from `$application_data_table` where group_id = ".$active_group_id;
	if(isset($_GET['season']) && $_GET['season'] != ""){
		 $sql_application_data .= " AND season_id=".$_GET['season'];
	}
	$application_data = $wpdb->get_results($sql_application_data);

	$sql_season = "select id, name, season_end from `$seasons_table` where users_groups_id = ".$active_group_id;
}else{


	$sql_season = "select id, name, season_end from `$seasons_table`";
	

}

$season_data = $wpdb->get_results($sql_season);
...
<div class="container main_user_manage form_list">
	<div class="main_user_manage_row">
			<div class="row-data-filter user_cols_main">
				...
				<div class="listing-outer" style="padding-right: 22px;width: 20%">
					<div class="search-box-inner">
						<select name="season" id="seasons">
							<option value="" data-link="<?php echo home_url( $wp->request );?>?season=<?php echo $season_d->id;?>">Alle sesonger</option>
							<?php foreach ($season_data as $key => $season_d) { ?>
							    <option value="<?php echo $season_d->id;?>" data-link="<?php echo home_url( $wp->request );?>?season=<?php echo $season_d->id;?>" <?php if($_GET["season"] == $season_d->id){ echo 'selected';}?>><?php echo $season_d->name;?></option>
							<?php } ?>
						</select>
					</div>
				</div> 
		...
		<div class="user_table_content" >
			<div class="table_div">
		        ...
		        <thead>
		            <tr>
		                <th class="group"><?php echo __("Søker","gibbs");?></th>
		                <th class="group"><?php echo __("Avdeling","gibbs");?></th>
		        ...
		        <tbody>
		        	...
		        	foreach ($application_data  as $key => $application) { 
		        		...
		        		$json_data = maybe_unserialize($application->json_data);
		        		...
		        		?>
			            <tr>
			                <td class="group"><?php echo $json_data["first_name"]." ".$json_data["last_name"];?></td>
			                        ...
			                        <div class="dropdown">
			                          ...
			                          <div id="listingDropdown" class="dropdown-content">
			                              <div class="outer-actions1">
			                              	...
			                              	<?php } ?>
			                              	<p data-link="<?php echo home_url();?>/application?application_id=<?php echo $application->id;?>&admin=true" class="open_form_link">Rediger <i class="fa fa-edit"></i></p>
...
jQuery("#seasons").change(function(){
	var linkk = jQuery(this).find("option:selected").attr("data-link");
...
jQuery(document).ready( function () {
	const dataJson = {
                   ...
                   // "pageLength" : 10,
			        ...
			        "sEmptyTable":    "Ingen har sendt søknad enda",
			        "sInfo":          "Viser _START_ til _END_ av _TOTAL_ søknader",
