	...
	public function insert( $args ) {
		...
		$args = wp_parse_args(
			...
			array(
				...
				'log_time'  => current_time( 'timestamp', 1 ),
			)
		);
  ...
  //       endif;
	...
	public function show_activities( $atts ){
		...
		$items = $wpdb->get_results( $wpdb->prepare(
			'SELECT log.* FROM '.$wpdb->prefix.'listeo_core_activity_log as log
			    LEFT JOIN '.$wpdb->prefix.'bookings_calendar AS calendar
				ON log.post_id = calendar.id
					WHERE log.related_to_id = '.$user_id.'
					AND (calendar.status != "payment_failed" AND calendar.status != "pay_to_confirm")
					GROUP BY log.id
		...
		foreach ($items as $key => $item) {
			...
			$item->minutes =  round(abs($current_time - $item->log_time) / 60,2);
			$datas[]= $item;


		}

		usort($datas, function($firstItem, $secondItem) {
	        $timeStamp1 = $firstItem->minutes;
	        $timeStamp2 = $secondItem->minutes;
		...
		ob_start(); ?>
			...
			<ul id="listeo-activities-list">
				...
				if($items) :
					...
					//$end = '  <strong><a href="'.get_author_posts_url($user_data->ID).'" class="name-user" style="margin-right:10px;margin-left:1px">'.$user_data->display_name.'</a></strong>';
					$end = '  <strong><a href="javascript:void(0)" class="name-user" style="margin-right:10px;margin-left:1px">'.$user_data->display_name.'</a></strong>';
					$end .= '<span class="activity-time">'.human_time_diff( $item->log_time, current_time('timestamp',1) ) . esc_html__(' ago','listeo_core').'</span></li>';

					/*$end .= '<a href="#" data-nonce="'.$nonce.'" data-id="'.$item->id.'" class="close-list-item" sty><i class="fa fa-close"></i></a></li>';*/
					...
					/* 		case 'bookmarked':
							...
							case 'listing_updated':
								echo $start.'
									<i class="fas fa-edit"></i> '.esc_html__('Listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was updated','listeo_core').'.
								'.$end;
								break;
							case 'listing_created':
								echo $start.'
									<i class="fas fa-check"></i> '.esc_html__('Listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was created.','listeo_core').$end;
								break;
							case 'listing_created':
								echo $start.'
									<i class="fas fa-check"></i>'.esc_html__('Your Listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was approved','listeo_core').'
								'.$end;
								break;
							case 'listing_trashed':
								echo $start.'
									<i class="fas fa-layer-group"></i> '.esc_html__('Listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was removed','listeo_core').'
								'.$end;
								break;
							...
							case 'approved':
								echo '
								<li>
									<i class="fas fa-layer-group"></i> '.esc_html__('Your Listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('has been approved!','listeo_core').'
							...
							case 'added':
								echo '
								<li>
									<i class="fas fa-layer-group"></i> '.esc_html__('You have added listing','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> 
									<a href="#" class="close-list-item"><i class="fa fa-close"></i></a>
								</li>';
								break;
							...
							case 'booking_created':
								echo $start.'
									<i class="fas fa-check"></i> '.esc_html__('Booking #').' <strong>'.$item->post_id.'</strong> '.esc_html__('ble kjøpt av','listeo_core').'
								'.$end;
								break;
							case 'booking_updated':
								echo $start.'
									<i class="fas fa-edit"></i> '.esc_html__('Ordre').' <strong>'.$item->post_id.'</strong> '.esc_html__('was updated','listeo_core').'.
								'.$end;
								break;
							case 'booking_updated':
								echo $start.'
							...
							case 'giftcard_created':
								echo $start.'
									<i class="fas fa-check"></i> '.esc_html__('Giftcard','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was created.','listeo_core').$end;
								break;
							case 'giftcard_updated':
									echo $start.'
										<i class="fas fa-edit"></i> '.esc_html__('Giftcard','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> '.esc_html__('was updated','listeo_core').$end;
									break;
							case 'giftcard_booking_created':
								echo $start.'
									<i class="fas fa-check"></i> Gavekort #'.' <strong>'.$item->post_id.'</strong> '.esc_html__('ble kjøpt av').'
								'.$end;
								break;				
							default:
								# code...
